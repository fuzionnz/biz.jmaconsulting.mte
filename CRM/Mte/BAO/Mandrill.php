    ...
    foreach ($reponse as $value) {
      ...
      if ($mail->find(TRUE)) {
        ...
        if ($eventQueueID) {
          ...
          // If activities_for is set to CiviMail (2), record an
          // activity. Could be better to switch this off for send
          // (not necessary) but record for other events (eg bounce,
          // unsub)?
          $mailingBackend = CRM_Core_BAO_Setting::getItem(CRM_Core_BAO_Setting::MAILING_PREFERENCES_NAME, 'mandrill_smtp_settings');
          // Civi::log()->debug(print_r($mailingBackend, 1));
          if (CRM_Utils_array::value('is_active', $mailingBackend) &&
            array_key_exists('activities_for', $mailingBackend) &&
            !empty($mailingBackend['activities_for'][2])) {
            Civi::log()->debug(print_r($mandrillActivtyParams, 1));
